...
%It includes the rules for which pairs are deemed compatible, the entry and death processes that govern the evolution of the pool, which characteristics are observable to the algorithm at each period, and the reward observed by the agent as sequentially matches and clears pairs. More rigorously, at each period an environment is described bu tuple $(G, u)$, where:

% 
% \begin{itemize}
%   \item $G = (V, A, X, \pi)$ is a graph with vertex set $V$, adjacency matrix $A$, vertex characteristic matrix $X$, and $\pi: \mathbb{N} \times V \rightarrow \{0, 1\}$ is a function that maps a period $t \in \mathbb{N}$ and a vertex $v \in V$ to whether that vertex was alive on period $t$ (1) or not (0). \footnote{We decided to model dynamic graphs in this manner as it is closer to our simulation code. Note that all the information about entries and exits can be expressed by the function $\pi$.}
%   \item $u: 2^V \rightarrow \mathbb{R} \cup \{-\infty\} $ is the reward associated with matching subset of $V$. Infeasible exchanges receive reward $-\infty$.
% \end{itemize}
